...
* **`.Names.Keep (list <strings>)`**: Keep these specific name strings exactly as-is (case-sensitive). Good for "Various Artists" or literal band names with commas.
...
* **`.Names.Role (dict <phrase> <canonical_role>)`**: Map trapped role annotations (e.g., `"(ed.)"` or `"[translator]"`) to canonical roles so they are extracted out of name strings.
...
### 1. Raw Column Values
To pull the cleaned raw string value of any CSV column, use the `.V` method:
```go
...
* **`required <error_msg> <value>`**: Fails and halts the conversion process with a user-friendly error message if the passed value is empty or nil.
* **`include <template_name> <context>`**: Renders another template inside the current template, allowing you to pipe the output:
  ...
  ```
* **`tpl <template_string> <context>`**: Compiles and executes an inline template string on the fly.
