...
// HyphenateISBN formats an ISBN with proper hyphens using the global hyphenator.
// The hyphenator is periodically refreshed with the latest ISBN agency ranges.
func HyphenateISBN(s string) string {
	return isbn.ISBN(s).Hyphenate()
}
...
// we are force to code <br/> into the text anyway.
