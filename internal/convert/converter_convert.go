...
func (c *Converter) convertAllEntries(ctx context.Context, reader io.Reader) iter.Seq2[Report, error] {
	return func(yield func(Report, error) bool) {
		...
		hyphenator, err := isbn.LoadHyphenator(ctx, c.httpClient)
		if err != nil {
			yield(Report{}, err)
			return
		}
		...
		extraFuncs := template.FuncMap{
			"isbn_hyphenator": isbnHyphenatorFunc(hyphenator),
			"trace":           func(msg string, args ...any) string { logger.Debug(msg, args...); return "" },
		}

		tmpl, err := Base(extraFuncs)
		if err != nil {
