# Readerware to Tellico Rescue Tool

A utility to audit and scrub CSV exports from Readerware for import into Tellico.
Developed as a "Digital Conservation" effort to move library data without loss.
...
Before you can run the conversion tool, you must export your collection data from Readerware into a CSV format.

### Step-by-Step Guide

1. **Open Readerware** and load the collection you wish to migrate (Books, Music, or Video).
2. **Select Export**: Navigate to the export menu option.
   
   ![Readerware File Menu](./docs/images/file_menu.png)
   
3. **Configure CSV Format**: Choose "CSV (Comma Separated Values)" as the target output format. Both "Output header line" and "Enclose fields in double quotes" must be selected; "Escape new lines" must not be selected. Press "Next".
   
   ![Export Format](./docs/images/export_format.png)
   
4. **Select Output File**: Choose the location and name of the output file. "Encoding" must be "UTF-8". Press "Next".
   
   ![File Selection](./docs/images/file_selection.png)
   
5. **Select Items to Export**: Choose the items to be exported. "All <items> in database" is usually the best choice. Press "Next".
...
9. **Done**: Review the export statistics and press "Finish".
...
3. **Configure Images Format**: Choose "Images" as the target output format and press "Next".
...
6. **Select Image to Export**: Choose which of the eight "Image" columns to export.  The "Image Format" should usually be "JPG".  The "Image Name" **must** be "Book ID".  Press "Next".
